...
import { ASTStorageCompatibilityReport, reportLayoutIncompatibilities } from "../../src/compatibility/ast-layout"
...
const selectReportFor = (report: ASTStorageCompatibilityReport[], contractName: string) => {
 ...
 */
const assertContractErrorsMatch = (report: ASTStorageCompatibilityReport[], contractName: string, expectedMatches: RegExp[]) => {
  const contractReport = selectReportFor(report, contractName)
  assert.equal(contractReport!.errors.length, 1)

  for (const [i, error] of contractReport!.errors.entries()) {
