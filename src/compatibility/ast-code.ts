...
function generateASTCompatibilityReport(oldContract: ZContract, oldArtifacts: BuildArtifacts,
  ...
  const old = stripMetadata(oldContract.schema.deployedBytecode)
  const neww = stripMetadata(newContract.schema.deployedBytecode)
  if (old !== neww) {
...
export function reportASTIncompatibilities(
  ...
  const reports = newArtifacts.listArtifacts()
    .map(newArtifact => {
      ...
      return generateASTCompatibilityReport(oldZContract as any, oldArtifacts, makeZContract(newArtifact), newArtifacts)
    })
